...
public class GoBackInteraction implements Interaction {

    @SneakyThrows
    @Override
    @Step("{0} go back from new task section")
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(WaitUntil.the(NewTaskPage.GO_BACK_BTN, isClickable()).forNoMoreThan(10).seconds());
        NewTaskPage.GO_BACK_BTN.resolveAllFor(theActorInTheSpotlight()).get(0).click();

        actor.attemptsTo(WaitUntil.the(HomePage.ADD_TASK_BTN, isClickable()).forNoMoreThan(10).seconds());

    }

    public static GoBackInteraction notSavedTask(NewTaskDataDto newTaskDataDto) {
        return instrumented(GoBackInteraction.class, newTaskDataDto);
    }
}
